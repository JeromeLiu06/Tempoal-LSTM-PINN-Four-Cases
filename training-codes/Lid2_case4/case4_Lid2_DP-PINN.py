...
SOURCE_CASE2_FILE = "ns2d_cavity_dp_pinn_rebuild_outputs (1).py"
...
class CavityConfig:
    ...
    out_dir: str = "./outputs/Case4-Lid2-Re100/DP-PINN"
    seed: int = 0
...
def save_model_info(cfg: CavityConfig, model: nn.Module) -> None:
    with open(os.path.join(cfg.out_dir, "model_info.txt"), "w", encoding="utf-8") as handle:
        ...
        handle.write("Architecture: DP-PINN recursive/shared operator backbone; hidden=128; recursive_steps=8; residual_scale=0.1\n")
