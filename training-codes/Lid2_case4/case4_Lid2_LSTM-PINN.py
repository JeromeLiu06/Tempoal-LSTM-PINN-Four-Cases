...
@dataclass
class StandardCavityConfig:
    case_name: str = "Lid-2"
    model_name: str = "LSTM-PINN-spatial-m4"
    ...
    fem_reference_path: str = (
        "./CFD参考解/case4_Lid2_Re100/final_reference/"
        "case4_fem_Re100_N512.npz"
    )

    out_dir: str = "./outputs/Lid-2/LSTM-PINN-spatial-m4"
    seed: int = 0


# =========================
# 2. High-fidelity FEM reference
# =========================

def load_fem_reference(path: str):
    if not os.path.exists(path):
        raise FileNotFoundError(
            "Case4 requires the independently generated FEM reference before "
            f"reference-based validation can begin. Missing: {path}"
        )
    d = np.load(path)
    for k in ["x", "y", "u", "v", "p"]:
        if k not in d:
            raise KeyError(f"FEM reference {path} does not contain key '{k}'")
    return {k: np.asarray(d[k]) for k in ["x", "y", "u", "v", "p"]}


def _fem_points_and_fields(ref):
    x, y = ref["x"], ref["y"]
    u, v, p = ref["u"], ref["v"], ref["p"]

    # Structured axes + 2-D fields.
    if x.ndim == 1 and y.ndim == 1 and u.size == x.size * y.size:
        XX, YY = np.meshgrid(x, y, indexing="ij")
        pts = np.column_stack([XX.reshape(-1), YY.reshape(-1)])
    else:
        if x.size != y.size:
            raise ValueError("Unsupported FEM format: x and y must be point arrays or structured axes.")
        pts = np.column_stack([x.reshape(-1), y.reshape(-1)])

    return pts, u.reshape(-1), v.reshape(-1), p.reshape(-1)


def interpolate_fem(ref, xq: np.ndarray, yq: np.ndarray):
    try:
        from scipy.interpolate import griddata
    except ImportError as e:
        raise ImportError("Case4 FEM interpolation requires scipy.") from e

    pts, u, v, p = _fem_points_and_fields(ref)
    q = np.column_stack([xq.reshape(-1), yq.reshape(-1)])
    result = {}

    for name, arr in [("u", u), ("v", v), ("p", p)]:
        z = griddata(pts, arr, q, method="linear")
        miss = np.isnan(z)
        if np.any(miss):
            z[miss] = griddata(pts, arr, q[miss], method="nearest")
        result[name] = z.reshape(xq.shape)

    result["p"] = result["p"] - np.mean(result["p"])
    result["speed"] = np.sqrt(result["u"] ** 2 + result["v"] ** 2)
    return result


def fem_reference_at_targets(cfg, ref, seq):
    x = seq[:, -1, 0].detach().cpu().numpy()
    y = seq[:, -1, 1].detach().cpu().numpy()
    q = interpolate_fem(ref, x, y)
    return (
        torch.as_tensor(q["u"].reshape(-1, 1), device=cfg.device, dtype=cfg.dtype),
        torch.as_tensor(q["v"].reshape(-1, 1), device=cfg.device, dtype=cfg.dtype),
        torch.as_tensor(q["p"].reshape(-1, 1), device=cfg.device, dtype=cfg.dtype),
    )


# =========================
# 3. Frozen pseudo-spatial dataset
# =========================

def _standard_bc(cfg, n, side, split_seed):
    ...
    seq = build_spatial_contexts(x, y, cfg.sequence_length)
    ur = torch.ones_like(x) if side == "top" else torch.zeros_like(x)
    vr = torch.zeros_like(x)
    a, b = split_indices(n, cfg.train_ratio, split_seed, cfg.device)
    return (seq[a], ur[a], vr[a]), (seq[b], ur[b], vr[b])


def build_dataset(cfg: StandardCavityConfig):
    # NOTE: FEM values are never used in the training objective.
    # They are used only for data diagnostics / validation metrics.
    ...
    b0_tr, b0_va = _standard_bc(cfg, cfg.n_bc_bottom, "bottom", cfg.seed + 201)
    b1_tr, b1_va = _standard_bc(cfg, cfg.n_bc_top, "top", cfg.seed + 202)
    bl_tr, bl_va = _standard_bc(cfg, cfg.n_bc_left, "left", cfg.seed + 203)
    br_tr, br_va = _standard_bc(cfg, cfg.n_bc_right, "right", cfg.seed + 204)

    return {
        "col_tr": (seq[a],), "col_va": (seq[b],),
        "bc_bottom_tr": b0_tr, "bc_bottom_va": b0_va,
        "bc_top_tr": b1_tr, "bc_top_va": b1_va,
...
def pde_parts(cfg, model, seq):
    ...
    # Conventional lid-driven cavity: NO manufactured forcing.
    ru = u * ux + v * uy + px - cfg.nu * (uxx + uyy)
    rv = u * vx + v * vy + py - cfg.nu * (vxx + vyy)
    div = ux + vy
...
def boundary_loss(model, bundle):
    # Standard no-slip/moving-lid velocity BC; pressure is NOT supervised on walls.
    seq, ur, vr = bundle
    o = model(seq)
    return (
...
def data_parts_from_fem(cfg, model, seq, ref):
    # Diagnostic only; NOT added to total training loss.
    o = model(seq)
    ur, vr, pr = fem_reference_at_targets(cfg, ref, seq)
    pp = o[:, 2:3] - torch.mean(o[:, 2:3])
    pr = pr - torch.mean(pr)
    sr = torch.sqrt(ur ** 2 + vr ** 2)
    sp = torch.sqrt(o[:, 0:1] ** 2 + o[:, 1:2] ** 2)
    lu = torch.mean((o[:, 0:1] - ur) ** 2)
    lv = torch.mean((o[:, 1:2] - vr) ** 2)
    lp = torch.mean((pp - pr) ** 2)
    ls = torch.mean((sp - sr) ** 2)
    return lu + lv + lp + ls, lu, lv, lp, ls


def total_loss(cfg, model, col, b0, b1, bl, br, fem_ref):
    lpde, lmom, ldiv = pde_parts(cfg, model, col[0])
    lbc = sum(boundary_loss(model, b) for b in [b0, b1, bl, br])
    ldata, lu, lv, lp, ls = data_parts_from_fem(cfg, model, col[0], fem_ref)
    total = lpde + cfg.w_bc * lbc
    z = torch.zeros((), device=cfg.device, dtype=cfg.dtype)
    return total, {
        "pde": lpde, "mom": lmom, "div": ldiv, "ic": z, "bc": lbc,
...
@torch.no_grad()
def evaluate_val_metrics(cfg, model, col_va, fem_ref):
    seq = col_va[0]
    o = model(seq)
    ur, vr, pr = fem_reference_at_targets(cfg, fem_ref, seq)
    pp = o[:, 2:3] - torch.mean(o[:, 2:3])
    pr = pr - torch.mean(pr)
    sr = torch.sqrt(ur ** 2 + vr ** 2)
    sp = torch.sqrt(o[:, 0:1] ** 2 + o[:, 1:2] ** 2)
    return compute_metrics_torch(
        torch.cat([o[:, 0:1], o[:, 1:2], pp, sp], dim=1),
        torch.cat([ur, vr, pr, sr], dim=1),
    )


@torch.no_grad()
def eval_on_grid(cfg, model, fem_ref):
    ...
    seq = build_spatial_contexts(X, Y, cfg.sequence_length)
    pred = model(seq)
    p_pred = pred[:, 2:3] - torch.mean(pred[:, 2:3])

    q = interpolate_fem(fem_ref, XX.cpu().numpy(), YY.cpu().numpy())
    uref = torch.as_tensor(q["u"], device=cfg.device, dtype=cfg.dtype)
    vref = torch.as_tensor(q["v"], device=cfg.device, dtype=cfg.dtype)
    pref = torch.as_tensor(q["p"], device=cfg.device, dtype=cfg.dtype)
    sref = torch.as_tensor(q["speed"], device=cfg.device, dtype=cfg.dtype)

    sp = torch.sqrt(pred[:, 0:1] ** 2 + pred[:, 1:2] ** 2)

    grid = {
        "x": xs.cpu().numpy(), "y": ys.cpu().numpy(),
        "u_ref": uref.cpu().numpy(),
        "v_ref": vref.cpu().numpy(),
        "p_ref": pref.cpu().numpy(),
        "speed_ref": sref.cpu().numpy(),
        "u_pred": pred[:, 0:1].view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "v_pred": pred[:, 1:2].view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "p_pred": p_pred.view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "speed_pred": sp.view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
    }
    for fld in ["u", "v", "p", "speed"]:
        grid[f"{fld}_err"] = grid[f"{fld}_pred"] - grid[f"{fld}_ref"]
    return grid
...
def save_grid_outputs(cfg, grid, history):
    ...
    folder = os.path.join(cfg.out_dir, "field_data", "steady")
    ensure_dir(folder)
    for fld in ["u", "v", "p", "speed"]:
        ...
        err = pred - ref
        np.savetxt(
            os.path.join(folder, f"{fld}_field_data.txt"),
            np.column_stack([
        ...
        rows.append({"case": cfg.case_name, "model": cfg.model_name, "time": "steady", "field": fld, **mm})
        all_pred.append(pred.reshape(-1))
        all_ref.append(ref.reshape(-1))

        save_map(ref, extent, f"FEM reference {fld}",
                 os.path.join(cfg.out_dir, "figures", "reference", f"{fld}_reference.png"))
        save_map(pred, extent, f"Prediction {fld}",
                 os.path.join(cfg.out_dir, "figures", "prediction", f"{fld}_prediction.png"))
        save_map(np.abs(err), extent, f"|{fld} error|",
                 os.path.join(cfg.out_dir, "figures", "error", f"{fld}_error.png"))

    midx = cfg.eval_nx // 2
    midy = cfg.eval_ny // 2
    plt.figure(figsize=(7, 4))
    plt.plot(grid["y"], grid["u_ref"][midx, :], label="FEM u @ x=0.5")
    plt.plot(grid["y"], grid["u_pred"][midx, :], "--", label="PINN u @ x=0.5")
    ...
    plt.figure(figsize=(7, 4))
    plt.plot(grid["x"], grid["v_ref"][:, midy], label="FEM v @ y=0.5")
    plt.plot(grid["x"], grid["v_pred"][:, midy], "--", label="PINN v @ y=0.5")
    plt.xlabel("x"); plt.ylabel("v"); plt.legend(); plt.tight_layout()
...
def train(cfg: StandardCavityConfig):
    ...
    save_model_info(
        cfg, model, "u, v, p",
        "steady pseudo-spatial Morton/Z-order context; t*=0 is nonphysical padding"
    )
    ...
    for it in range(1, cfg.iters + 1):
        ...
        b0 = deterministic_minibatch(data["bc_bottom_tr"], cfg.batch_bc, cfg.seed + 10000 + it)
        b1 = deterministic_minibatch(data["bc_top_tr"], cfg.batch_bc, cfg.seed + 20000 + it)
        bl = deterministic_minibatch(data["bc_left_tr"], cfg.batch_bc, cfg.seed + 30000 + it)
        br = deterministic_minibatch(data["bc_right_tr"], cfg.batch_bc, cfg.seed + 40000 + it)

        loss, parts = total_loss(cfg, model, col, b0, b1, bl, br, fem_ref)
        ...
        if it == 1 or it % cfg.eval_every == 0:
            model.eval()
            colv = deterministic_minibatch(data["col_va"], cfg.val_batch_col, cfg.seed + 50000 + it)
            b0v = deterministic_minibatch(data["bc_bottom_va"], cfg.val_batch_bc, cfg.seed + 60000 + it)
            b1v = deterministic_minibatch(data["bc_top_va"], cfg.val_batch_bc, cfg.seed + 70000 + it)
            ...
            lv, pv = total_loss(cfg, model, colv, b0v, b1v, blv, brv, fem_ref)
            vm = evaluate_val_metrics(cfg, model, colv, fem_ref)
            ...
            row = {
                ...
                "loss_ic_train": 0.0, "loss_ic_val": 0.0,
    ...
    model.eval()
    grid = eval_on_grid(cfg, model, fem_ref)
