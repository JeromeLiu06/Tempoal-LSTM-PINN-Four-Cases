...
SOURCE_CASE2_FILE = "ns2d_cavity_piratenet_rebuild_outputs.py"
...
class CavityConfig:
    ...
    # Exact Case2 PirateNet-style settings.
    hidden: int = 128
    ...
    out_dir: str = "./outputs/Case4-Lid2-Re100/PirateNet"
    seed: int = 0
...
def save_model_info(cfg: CavityConfig, model: nn.Module) -> None:
    with open(os.path.join(cfg.out_dir, "model_info.txt"), "w", encoding="utf-8") as handle:
        ...
        handle.write("Architecture: PirateNet-style gated residual PINN; hidden=128; gated_residual_blocks=8\n")
