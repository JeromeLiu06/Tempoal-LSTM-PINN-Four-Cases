...
SOURCE_CASE2_FILE = "ns2d_cavity_param_ann_pinn_rebuild_outputs.py"
...
class CavityConfig:
    ...
    # Exact Case2 parameter-matched ANN settings.
    hidden: int = 280
    ...
    out_dir: str = "./outputs/Case4-Lid2-Re100/Parameter-ANN-PINN"
    seed: int = 0
...
def save_model_info(cfg: CavityConfig, model: nn.Module) -> None:
    with open(os.path.join(cfg.out_dir, "model_info.txt"), "w", encoding="utf-8") as handle:
        ...
        handle.write("Architecture: Parameter-matched ANN-PINN; hidden=280; hidden_layers=9; target_lstm_parameters=630915\n")
