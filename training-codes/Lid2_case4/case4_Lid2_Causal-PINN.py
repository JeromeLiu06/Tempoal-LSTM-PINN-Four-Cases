...
SOURCE_CASE2_FILE = "ns2d_cavity_causal_pinn_rebuild_outputs.py"
...
class CavityConfig:
    ...
    # Exact Case2 causal PINN settings.
    hidden: int = 128
    ...
    out_dir: str = "./outputs/Case4-Lid2-Re100/Causal-PINN"
    seed: int = 0
...
def causal_spatial_weighted_losses(
    ...
    """Exact Case2 causal chunking/weighting, applied to the Case4 zero-forcing residual."""
    ...
    chunk_mom_losses: List[torch.Tensor] = []
    chunk_div_losses: List[torch.Tensor] = []
    for index in range(cfg.causal_chunks):
        left, right = edges[index], edges[index + 1]
        mask = ((coord_flat >= left) & (coord_flat <= right)) if index == cfg.causal_chunks - 1 else (
            (coord_flat >= left) & (coord_flat < right)
        )
        if torch.any(mask):
            chunk_mom_losses.append(torch.mean(mom_flat[mask]))
            chunk_div_losses.append(torch.mean(div_flat[mask]))
    if not chunk_mom_losses:
        loss_mom = torch.mean(point_mom)
        loss_div = torch.mean(point_div)
        return loss_mom + loss_div, loss_mom, loss_div

    weighted_mom: List[torch.Tensor] = []
    weighted_div: List[torch.Tensor] = []
    cumulative = torch.zeros((), device=coord.device, dtype=coord.dtype)
    for mom_loss, div_loss in zip(chunk_mom_losses, chunk_div_losses):
        weight = torch.exp(-cfg.causal_epsilon * cumulative.detach())
        weighted_mom.append(weight * mom_loss)
        weighted_div.append(weight * div_loss)
        cumulative = cumulative + mom_loss + div_loss
...
def batch_losses(
    ...
    with torch.enable_grad():
        ...
        point_div = r_div ** 2
        loss_pde, loss_mom, loss_div = causal_spatial_weighted_losses(
            point_mom, point_div, x, y, cfg
        )
...
def save_model_info(cfg: CavityConfig, model: nn.Module) -> None:
    with open(os.path.join(cfg.out_dir, "model_info.txt"), "w", encoding="utf-8") as handle:
        ...
        handle.write("Architecture: Causal-PINN MLP; hidden=128; hidden_layers=8; chunks=10; epsilon=1.0; axis=y\n")
