...
class CavityConfig:
    ...
    # Causal-PINN uses the same MLP capacity as the vanilla PINN baseline.
    # For this steady Lid case, temporal causal weighting is not physically defined;
    # therefore, a shape-safe ordered residual curriculum is applied over spatial chunks.
    hidden: int = 128
...
class BaselineModel(nn.Module):
    """
    Causal-PINN-style baseline for the lid-driven cavity case.

    The network architecture is intentionally identical to the vanilla PINN
    baseline. Since the cavity case is steady and has no time coordinate, the
    causal component is implemented only as an ordered residual curriculum over
    spatial chunks in the PDE residual aggregation.
    """
...
def causal_spatial_weighted_losses(
    ...
    """
    Compute ordered residual-curriculum weighted PDE losses for the steady Lid case.

    The original causality-aware PINN weighting is temporal. The lid-driven cavity
    MMS benchmark here is steady, so there is no physical time direction. To keep
    the seven-model comparison structurally complete without changing the dataset
    or training budget, this implementation applies the same exponential
    accumulated-residual idea over ordered spatial chunks. The selected coordinate
    is controlled by cfg.causal_axis.
    """
