# ns2d_cavity_param_ann_pinn_rebuild_outputs.py
# Rebuilt parameter-matched ANN-PINN for 2D incompressible Navier-Stokes lid-driven cavity MMS benchmark.
# Main purpose:
...
class CavityConfig:
    ...
    # Width/depth selected to match the Lid LSTM-PINN parameter count (630,915).
    hidden: int = 280
    num_hidden_layers: int = 9
    target_lstm_parameters: int = 630915
    ...
    # Output
    out_dir: str = "./outputs/Lid/Parameter-ANN-PINN"
    seed: int = 0
...
class BaselineModel(nn.Module):
    """Parameter-matched wider/deeper feedforward ANN-PINN baseline for the lid-driven cavity case."""
...
def save_model_info(cfg: CavityConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: Parameter-matched wider/deeper fully connected ANN-PINN\n")
        ...
        current_params = count_trainable_parameters(model)
        rel_diff = abs(current_params - cfg.target_lstm_parameters) / cfg.target_lstm_parameters
        f.write(f"Relative parameter difference to LSTM-PINN: {rel_diff:.8f}\n")
        f.write(f"Trainable parameters: {current_params}\n")
...
def save_cost_outputs(
    ...
    summary = {
        ...
        "relative_param_diff_to_lstm": abs(count_trainable_parameters(model) - cfg.target_lstm_parameters) / cfg.target_lstm_parameters,
