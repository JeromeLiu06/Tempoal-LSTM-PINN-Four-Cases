...
class CavityConfig:
    ...
    out_dir: str = "./outputs/Lid/LSTM-PINN-spatial-m4"
    seed: int = 0


# =========================
# 2. Lid manufactured reference
# =========================

def fpoly(x): return x ** 2 - 2.0 * x ** 3 + x ** 4
def fpoly_p(x): return 2.0 * x - 6.0 * x ** 2 + 4.0 * x ** 3
def fpoly_pp(x): return 2.0 - 12.0 * x + 12.0 * x ** 2
def fpoly_ppp(x): return -12.0 + 24.0 * x
def gpoly(y): return y ** 3 - y ** 2
def gpoly_p(y): return 3.0 * y ** 2 - 2.0 * y
def gpoly_pp(y): return 6.0 * y - 2.0
...
def forcing_mms(x, y, nu):
    ...
    fu = u * ux + v * uy + px - nu * (uxx + uyy)
    fv = u * vx + v * vy + py - nu * (vxx + vyy)
    return fu, fv
...
def _make_bc(cfg, n, side, split_seed):
    ...
    ur, vr, pr = uvp_mms(x, y)
    tr, va = split_indices(n, cfg.train_ratio, split_seed, cfg.device)
    train = (seq[tr], ur[tr].detach(), vr[tr].detach(), pr[tr].detach())
    val = (seq[va], ur[va].detach(), vr[va].detach(), pr[va].detach())
    return train, val


def build_dataset(cfg: CavityConfig):
    ...
    b0_tr, b0_va = _make_bc(cfg, cfg.n_bc_bottom, "bottom", cfg.seed + 201)
    b1_tr, b1_va = _make_bc(cfg, cfg.n_bc_top, "top", cfg.seed + 202)
    bl_tr, bl_va = _make_bc(cfg, cfg.n_bc_left, "left", cfg.seed + 203)
    br_tr, br_va = _make_bc(cfg, cfg.n_bc_right, "right", cfg.seed + 204)
...
def pde_parts(cfg, model, seq):
    ...
    x = seq[:, -1, 0:1]
    y = seq[:, -1, 1:2]
    fu, fv = forcing_mms(x, y, cfg.nu)

    ru = u * ux + v * uy + px - cfg.nu * (uxx + uyy) - fu
    rv = u * vx + v * vy + py - cfg.nu * (vxx + vyy) - fv
    div = ux + vy
...
def total_loss(cfg, model, col, b0, b1, bl, br):
...
def eval_on_grid(cfg, model):
    ...
    grid = {
        ...
        "u_ref": ur.view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "v_ref": vr.view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "p_ref": pr.view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "speed_ref": sr.view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "u_pred": pred[:, 0:1].view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "v_pred": pred[:, 1:2].view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
        "p_pred": pred[:, 2:3].view(cfg.eval_nx, cfg.eval_ny).cpu().numpy(),
...
def save_grid_outputs(cfg, grid, history):
    ...
    for fld in ["u", "v", "p", "speed"]:
        ...
        save_map(ref, extent, f"Reference {fld}",
    ...
    # Centerline diagnostics retained to match the cavity output style.
    midx = cfg.eval_nx // 2
    midy = cfg.eval_ny // 2
    plt.figure(figsize=(7, 4))
    plt.plot(grid["y"], grid["u_ref"][midx, :], label="u reference @ x=0.5")
    plt.plot(grid["y"], grid["u_pred"][midx, :], "--", label="u prediction @ x=0.5")
    ...
    plt.figure(figsize=(7, 4))
    plt.plot(grid["x"], grid["v_ref"][:, midy], label="v reference @ y=0.5")
    plt.plot(grid["x"], grid["v_pred"][:, midy], "--", label="v prediction @ y=0.5")
...
def train(cfg: CavityConfig):
    ...
    for it in range(1, cfg.iters + 1):
        ...
        loss, parts = total_loss(cfg, model, col, b0, b1, bl, br)
        ...
        if it == 1 or it % cfg.eval_every == 0:
            ...
            lv, pv = total_loss(cfg, model, colv, b0v, b1v, blv, brv)
