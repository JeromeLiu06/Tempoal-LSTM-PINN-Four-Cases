...
# Rebuilt ResAtten-PINN for 2D incompressible Navier-Stokes lid-driven cavity MMS benchmark.
# Main purpose:
...
class CavityConfig:
    ...
    # ResAtten-PINN architecture. Keep width/depth/training settings aligned with other Lid baselines.
    hidden: int = 128
    ...
    # Output
    out_dir: str = "./outputs/Lid/ResAtten-PINN"
    seed: int = 0
...
class ResidualAttentionBlock(nn.Module):
    """Residual-attention block used as a strong PINN backbone baseline.

    The block keeps the same hidden width as the vanilla PINN baseline, but adds
    a residual branch and a sigmoid attention gate. No recurrent memory is used.
    """
...
class BaselineModel(nn.Module):
    """ResAtten-PINN baseline for the lid-driven cavity case."""
    ...
    def __init__(self, cfg: CavityConfig):
        ...
        self.blocks = nn.ModuleList([
            ResidualAttentionBlock(cfg.hidden) for _ in range(cfg.num_hidden_layers)
        ])
...
def save_model_info(cfg: CavityConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: Residual-attention PINN backbone\n")
