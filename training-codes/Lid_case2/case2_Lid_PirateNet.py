# ns2d_cavity_reatten_pinn_rebuild_outputs.py
# Rebuilt PirateNet for 2D incompressible Navier-Stokes lid-driven cavity MMS benchmark.
# Main purpose:
...
class CavityConfig:
    ...
    # PirateNet-style architecture. Keep width/depth/training settings aligned with other Lid baselines.
    hidden: int = 128
    ...
    # Output
    out_dir: str = "./outputs/Lid/PirateNet"
    seed: int = 0
...
class PirateBlock(nn.Module):
    """PirateNet-style gated residual block used as a strong feedforward PINN baseline.

    This implementation follows the practical spirit of PirateNet-type backbones:
    repeated residual transformations with multiplicative gates are used to improve
    optimization while keeping the training data, PDE residuals, and output protocol
    identical to the other Lid baselines. No recurrent memory is used.
    """
...
class BaselineModel(nn.Module):
    """PirateNet-style PINN baseline for the lid-driven cavity case."""
...
def save_model_info(cfg: CavityConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: PirateNet-style gated residual PINN backbone\n")
        ...
        f.write("Activation: Tanh with sigmoid residual gates\n")
