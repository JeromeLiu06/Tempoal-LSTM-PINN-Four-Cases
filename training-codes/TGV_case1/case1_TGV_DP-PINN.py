# ns2d_tgv_dp_pinn_rebuild_outputs.py
# Rebuilt DP-PINN for 2D incompressible Navier-Stokes Taylor-Green MMS benchmark.
# Main purpose:
...
class NS2DConfig:
    ...
    # DP-PINN architecture: shared recursive operator backbone
    hidden: int = 128
    ...
    # Output
    out_dir: str = "./outputs/TGV/DP-PINN"
    seed: int = 0
...
class RecursiveOperatorBlock(nn.Module):
    """
    Shared recursive operator block used by DP-PINN.

    The same nonlinear operator is applied repeatedly to the latent state.
    This keeps the architecture distinct from a plain MLP while preserving
    the same training data, optimizer, loss terms, and output protocol used
    by the other TGV baselines.
    """
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        return self.operator(h)


class BaselineModel(nn.Module):
    """
    DP-PINN baseline with a recursive/shared operator backbone.

    Input:  (x, y, t)
    Output: (u, v, p)

    Compared with the vanilla PINN, this model replaces a stack of independent
    hidden layers with a shared recursive operator applied for a fixed number
    of steps. No LSTM memory, causal weighting, or attention gates are used.
    """
    def __init__(self, cfg: NS2DConfig):
        ...
        self.recursive_operator = RecursiveOperatorBlock(cfg.hidden)
        self.output_layer = nn.Linear(cfg.hidden, 3)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, t: torch.Tensor):
        ...
        for _ in range(self.recursive_steps):
            h = h + self.residual_scale * self.recursive_operator(h)
...
def save_model_info(cfg: NS2DConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: DP-PINN recursive/shared operator backbone\n")
        f.write(f"Input variables: x, y, t\n")
        f.write(f"Output fields: u, v, p\n")
        f.write(f"Hidden width: {cfg.hidden}\n")
        f.write(f"Nominal recursive steps: {cfg.recursive_steps}\n")
        f.write(f"Residual scale: {cfg.residual_scale}\n")
        f.write("Activation: Tanh\n")
        f.write("Memory mechanism: None\n")
        f.write("Attention mechanism: None\n")
        f.write("Causal weighting: False\n")
