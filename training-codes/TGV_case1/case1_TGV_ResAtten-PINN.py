# ns2d_tgv_reatten_pinn_rebuild_outputs.py
# Rebuilt Residual-Attention PINN for 2D incompressible Navier-Stokes Taylor-Green MMS benchmark.
# Main purpose:
...
class NS2DConfig:
    ...
    # Output
    out_dir: str = "./outputs/TGV/ResAtten-PINN"
    seed: int = 0
...
class ResidualAttentionBlock(nn.Module):
    """
    Residual-attention block used in the ResAtten-PINN baseline.

    The block keeps a feedforward PINN structure but augments each hidden
    transformation with a learned elementwise attention gate. It does not
    introduce recurrent temporal memory, so it remains a structural baseline
    distinct from LSTM-PINN.
    """
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        z = self.activation(self.feature(h))
        a = self.attention(h)
        return h + a * z
...
class BaselineModel(nn.Module):
    """
    Residual-Attention PINN baseline for the TGV benchmark.

    Mapping: (x, y, t) -> (u, v, p).
    The model uses an input projection, a stack of residual-attention blocks,
    and an output projection. All loss definitions, sampling, metrics, and
    computational-cost outputs are kept identical to the other TGV models.
    """
    def __init__(self, cfg: NS2DConfig):
        ...
        self.blocks = nn.ModuleList([
            ResidualAttentionBlock(cfg.hidden)
...
def save_model_info(cfg: NS2DConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: residual-attention feedforward PINN\n")
        f.write(f"Input variables: x, y, t\n")
        f.write(f"Output fields: u, v, p\n")
        f.write(f"Hidden width: {cfg.hidden}\n")
        f.write(f"Residual-attention blocks: {cfg.num_hidden_layers}\n")
        f.write("Activation: Tanh with sigmoid attention gates\n")
