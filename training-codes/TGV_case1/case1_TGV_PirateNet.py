...
class NS2DConfig:
    ...
    # Output
    out_dir: str = "./outputs/TGV/PirateNet"
    seed: int = 0
...
class PirateBlock(nn.Module):
    ...
    This implementation keeps the training setup aligned with the other TGV
    baselines while providing a stronger residual/gated feedforward backbone.
    The block contains two nonlinear transformations and a learned residual
    gate. It does not introduce recurrent memory, so it remains distinct from
    LSTM-PINN.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        residual_scale = torch.sigmoid(self.alpha)
        return x + residual_scale * g * h


class BaselineModel(nn.Module):
    """
    PirateNet-style PINN baseline.

    The model maps (x, y, t) to (u, v, p) using a feedforward residual-gated
    backbone. The PDE, sampling, optimizer, loss weights, batch sizes, and all
    standardized outputs are kept identical to the other TGV baselines. This
    makes the comparison isolate the effect of the stronger PirateNet-style
    architecture rather than changes in the training protocol.
    """
    def __init__(self, cfg: NS2DConfig):
        super().__init__()
        self.input_layer = nn.Linear(3, cfg.hidden)
...
def save_model_info(cfg: NS2DConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: PirateNet-style gated residual PINN\n")
        ...
        f.write("Block type: gated residual feedforward block\n")
