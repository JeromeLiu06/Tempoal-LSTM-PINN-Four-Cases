# ns2d_tgv_param_ann_pinn_rebuild_outputs.py
# Rebuilt parameter-matched ANN-PINN for 2D incompressible Navier-Stokes Taylor-Green MMS benchmark.
# Main purpose:
...
class NS2DConfig:
    ...
    # Selected to match TGV LSTM-PINN parameters (630,915) as closely as possible.
    hidden: int = 280
    num_hidden_layers: int = 9
    matched_to_model: str = "LSTM-PINN"
    target_lstm_params: int = 630915
    ...
    # Output
    out_dir: str = "./outputs/TGV/Parameter-ANN-PINN"
    seed: int = 0
...
class BaselineModel(nn.Module):
    """
    Parameter-matched feedforward ANN-PINN baseline.

    This is a deliberately widened and deepened fully connected MLP mapping
    (x, y, t) to (u, v, p). It contains no recurrent, attention, memory,
    decomposition, or causal weighting module. Its hidden width/depth are
    selected so that its trainable parameter count is highly aligned with
    the TGV LSTM-PINN baseline, isolating model capacity from temporal memory.
    """
...
def save_model_info(cfg: NS2DConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: parameter-matched widened/deepened fully connected ANN-PINN\n")
        ...
        f.write(f"Matched to model: {cfg.matched_to_model}\n")
        f.write(f"Target LSTM-PINN parameters: {cfg.target_lstm_params}\n")
        f.write(f"Trainable parameters: {trainable_params}\n")
        f.write(f"Relative parameter difference to LSTM-PINN: {abs(trainable_params - cfg.target_lstm_params) / cfg.target_lstm_params:.8f}\n")
