...
class NS2DConfig:
    ...
    # Causal-PINN architecture: intentionally aligned with vanilla PINN.
    hidden: int = 128
    num_hidden_layers: int = 8

    # Causal training parameters for temporal residual weighting.
    # The network capacity, optimizer, sampling sizes, and training iterations are kept
    # identical to the vanilla PINN baseline; only the PDE residual aggregation changes.
    ...
    # Output
    out_dir: str = "./outputs/TGV/Causal-PINN"
    seed: int = 0
...
class BaselineModel(nn.Module):
    """
    Feedforward neural network used by Causal-PINN.

    The architecture is deliberately identical to the vanilla PINN baseline:
    a fully connected MLP mapping (x, y, t) to (u, v, p). The causal component
    is implemented only in the training loss through temporal residual weighting,
    so the comparison isolates the effect of causality-aware optimization rather
...
def causal_time_weighted_losses(
    ...
    """
    Compute causal weighted PDE losses over temporal chunks.

    The time domain is split into fixed intervals. The residual loss at each
    later interval is exponentially down-weighted by the accumulated residual
    of earlier intervals, following the causality-aware training principle.
    Empty chunks are skipped, which keeps minibatch training shape-safe.
    """
    t_flat = t.reshape(-1)
    ...
    for lm, ld in zip(chunk_mom_losses, chunk_div_losses):
        # The weighting factor is detached from the current optimization graph.
        # This keeps the causal schedule stable and avoids introducing higher-order
        # interactions between different temporal chunks.
...
def batch_losses(cfg: NS2DConfig, model: BaselineModel, col, ic, bcx, bcy):
    with torch.enable_grad():
        ...
        point_div = r_div ** 2
        loss_pde, loss_mom, loss_div = causal_time_weighted_losses(point_mom, point_div, t, cfg)
...
def save_model_info(cfg: NS2DConfig, model: nn.Module):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: fully connected MLP PINN\n")
