# ns2d_rbc_dp_pinn_rebuild_outputs.py
# Rebuilt DP-PINN training code for 2D Rayleigh-Benard Convection MMS benchmark
...
class RBCConfig:
    ...
    # DP-PINN architecture
    hidden: int = 128
    recursive_steps: int = 8
    ...
    out_dir: str = "./outputs/RBC/DP-PINN"
    seed: int = 0
...
def forcing_mms_detached(x: torch.Tensor, y: torch.Tensor, t: torch.Tensor, cfg: RBCConfig):
    """Manufactured forcing terms for RBC equations, detached for PINN residual."""
    x = ensure_col(x)
    y = ensure_col(y)
    t = ensure_col(t)
...
class RecursiveOperatorBlock(nn.Module):
    """Shared recursive operator block used in DP-PINN."""
    def __init__(self, hidden: int, residual_scale: float = 0.1):
        super().__init__()
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        return z + self.residual_scale * torch.tanh(self.op(z))


class DPPINNModel(nn.Module):
    """
    DP-PINN / recursive-operator PINN baseline.

    The input/output interface is identical to the standard RBC PINN: 
    (x, y, t) -> (u, v, p, theta). Only the backbone is changed to a
    shared recursive operator to preserve the unified training and output
    protocol across all seven models.
    """
    def __init__(self, cfg: RBCConfig):
        super().__init__()
        self.input_layer = nn.Sequential(
            nn.Linear(3, cfg.hidden),
            nn.Tanh(),
        )
        self.recursive_operator = RecursiveOperatorBlock(cfg.hidden, cfg.residual_scale)
        self.output_layer = nn.Linear(cfg.hidden, 4)
        self.recursive_steps = cfg.recursive_steps
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, t: torch.Tensor):
        ...
        z = self.input_layer(inp)
        for _ in range(self.recursive_steps):
            z = self.recursive_operator(z)
...
def batch_losses(cfg: RBCConfig, model: DPPINNModel, col, ic, bc_left, bc_right, bc_bottom, bc_top):
    with torch.enable_grad():
        ...
        loss_theta_pde = torch.mean(r_th ** 2)
        loss_pde = loss_mom + loss_theta_pde
...
@torch.no_grad()
def data_loss_fields(cfg: RBCConfig, model: DPPINNModel, batch) -> Dict[str, float]:
    x, y, t = batch
...
@torch.no_grad()
def evaluate_val_metrics(cfg: RBCConfig, model: DPPINNModel, data, n: int = 3000) -> Dict[str, float]:
...
@torch.no_grad()
def eval_on_grid(cfg: RBCConfig, model: DPPINNModel):
...
def write_metrics(cfg: RBCConfig, grid: Dict[str, Any]):
    ...
    # Overall metrics are computed over primary output fields u, v, p, theta across all saved times.
...
def write_model_info(cfg: RBCConfig, model: nn.Module, trainable_params: int, total_params: int):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: DP-PINN / shared recursive operator backbone\n")
        f.write("Input dimension: 3 (x, y, t)\n")
        f.write("Output dimension: 4 (u, v, p, theta)\n")
        f.write(f"Hidden width: {cfg.hidden}\n")
        f.write(f"Recursive steps: {cfg.recursive_steps}\n")
        f.write(f"Residual scale: {cfg.residual_scale}\n")
...
def train(cfg: RBCConfig):
    ...
    model = DPPINNModel(cfg).to(cfg.device, cfg.dtype)
