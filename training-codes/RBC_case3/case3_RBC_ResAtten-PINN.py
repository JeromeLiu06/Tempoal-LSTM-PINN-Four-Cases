# ns2d_rbc_reatten_pinn_rebuild_outputs.py
# Rebuilt ResAtten-PINN training code for 2D Rayleigh-Benard Convection MMS benchmark
...
class RBCConfig:
    ...
    out_dir: str = "./outputs/RBC/ResAtten-PINN"
    seed: int = 0
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        ...
        gate = self.attention(h)
        return h + gate * feature


class ResAttenPINNModel(nn.Module):
    """
    Residual-attention PINN backbone for the RBC benchmark.

    The training data, PDE residuals, optimizer, loss weights, and output
    standard are kept identical to the RBC PINN baseline. Only the neural
    network backbone is replaced by residual-attention blocks.
    """
    def __init__(self, cfg: RBCConfig):
        ...
        self.blocks = nn.ModuleList([ResidualAttentionBlock(cfg.hidden) for _ in range(cfg.layers)])
...
def batch_losses(cfg: RBCConfig, model: ResAttenPINNModel, col, ic, bc_left, bc_right, bc_bottom, bc_top):
    with torch.enable_grad():
...
@torch.no_grad()
def data_loss_fields(cfg: RBCConfig, model: ResAttenPINNModel, batch) -> Dict[str, float]:
    x, y, t = batch
...
@torch.no_grad()
def evaluate_val_metrics(cfg: RBCConfig, model: ResAttenPINNModel, data, n: int = 3000) -> Dict[str, float]:
...
@torch.no_grad()
def eval_on_grid(cfg: RBCConfig, model: ResAttenPINNModel):
...
def write_model_info(cfg: RBCConfig, model: nn.Module, trainable_params: int, total_params: int):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Residual-attention blocks: {cfg.layers}\n")
...
def train(cfg: RBCConfig):
    ...
    model = ResAttenPINNModel(cfg).to(cfg.device, cfg.dtype)
