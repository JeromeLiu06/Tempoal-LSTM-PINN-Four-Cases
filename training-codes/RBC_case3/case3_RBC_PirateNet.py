# ns2d_rbc_piratenet_rebuild_outputs.py
# Rebuilt PirateNet training code for 2D Rayleigh-Benard Convection MMS benchmark
...
class RBCConfig:
    ...
    # PirateNet-style architecture
    hidden: int = 128
    num_pirate_blocks: int = 8
    ...
    out_dir: str = "./outputs/RBC/PirateNet"
    seed: int = 0
...
class PirateBlock(nn.Module):
    ...
    This block provides a strong feedforward PINN backbone with repeated
    nonlinear transformations and multiplicative residual gates. The training
    data, PDE residuals, optimizer, loss weights, and output protocol are kept
    identical to the other RBC baselines. No recurrent memory is used.
    """
...
class PirateNetModel(nn.Module):
    """PirateNet-style PINN backbone for the RBC benchmark."""
    def __init__(self, cfg: RBCConfig):
        ...
        self.blocks = nn.ModuleList([
            PirateBlock(cfg.hidden) for _ in range(cfg.num_pirate_blocks)
        ])
...
def batch_losses(cfg: RBCConfig, model: PirateNetModel, col, ic, bc_left, bc_right, bc_bottom, bc_top):
    with torch.enable_grad():
...
@torch.no_grad()
def data_loss_fields(cfg: RBCConfig, model: PirateNetModel, batch) -> Dict[str, float]:
    x, y, t = batch
...
@torch.no_grad()
def evaluate_val_metrics(cfg: RBCConfig, model: PirateNetModel, data, n: int = 3000) -> Dict[str, float]:
...
@torch.no_grad()
def eval_on_grid(cfg: RBCConfig, model: PirateNetModel):
...
def write_model_info(cfg: RBCConfig, model: nn.Module, trainable_params: int, total_params: int):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: residual-attention PINN\n")
        f.write("Input dimension: 3 (x, y, t)\n")
        f.write("Output dimension: 4 (u, v, p, theta)\n")
        f.write(f"Hidden width: {cfg.hidden}\n")
        f.write(f"Pirate blocks: {cfg.num_pirate_blocks}\n")
        f.write("Activation: Tanh + sigmoid attention gates\n")
...
def train(cfg: RBCConfig):
    ...
    model = PirateNetModel(cfg).to(cfg.device, cfg.dtype)
