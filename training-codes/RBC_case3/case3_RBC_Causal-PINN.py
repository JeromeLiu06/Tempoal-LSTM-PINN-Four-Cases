# ns2d_rbc_causal_pinn_rebuild_outputs.py
# Rebuilt Causal-PINN training code for 2D Rayleigh-Benard Convection MMS benchmark
...
def causal_time_weighted_losses(
    ...
    """
    Causality-aware residual aggregation over temporal chunks.

    Earlier temporal chunks are optimized first. Later chunks are down-weighted
    when accumulated previous residuals are still large. This follows the
    causality-aware PINN principle while keeping the same network capacity and
    training hyperparameters as the standard PINN baseline.
    """
    ...
    for lm, lt, ld in zip(chunk_mom_losses, chunk_theta_losses, chunk_div_losses):
        # Detach cumulative residual so causal weights do not introduce unwanted
        # cross-chunk higher-order interactions.
