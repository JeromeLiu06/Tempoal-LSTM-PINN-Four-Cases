...
class RBCConfig:
    case_name: str = "RBC"
    model_name: str = "Parameter-ANN-PINN"
    ...
    # Parameter-matched ANN-PINN architecture
    # Wider and deeper feedforward baseline matched to RBC LSTM-PINN parameters.
    hidden: int = 280
    layers: int = 9
    target_lstm_parameters: int = 631108
    ...
    out_dir: str = "./outputs/RBC/Parameter-ANN-PINN"
    seed: int = 0
...
class ParameterANNPINNModel(nn.Module):
...
def batch_losses(cfg: RBCConfig, model: ParameterANNPINNModel, col, ic, bc_left, bc_right, bc_bottom, bc_top):
    with torch.enable_grad():
...
@torch.no_grad()
def data_loss_fields(cfg: RBCConfig, model: ParameterANNPINNModel, batch) -> Dict[str, float]:
    x, y, t = batch
...
@torch.no_grad()
def evaluate_val_metrics(cfg: RBCConfig, model: ParameterANNPINNModel, data, n: int = 3000) -> Dict[str, float]:
...
@torch.no_grad()
def eval_on_grid(cfg: RBCConfig, model: ParameterANNPINNModel):
...
def write_model_info(cfg: RBCConfig, model: nn.Module, trainable_params: int, total_params: int):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Architecture: parameter-matched wider/deeper feedforward ANN-PINN / MLP\n")
        ...
        f.write(f"Target LSTM-PINN parameters: {cfg.target_lstm_parameters}\n")
        f.write(f"Relative parameter difference to LSTM-PINN: {abs(trainable_params - cfg.target_lstm_parameters) / cfg.target_lstm_parameters:.6%}\n")
...
def write_cost_summary(
    ...
    row = {
        ...
        "matched_to_model": "LSTM-PINN",
        "target_lstm_parameters": cfg.target_lstm_parameters,
        "relative_param_diff_to_lstm": abs(trainable_params - cfg.target_lstm_parameters) / cfg.target_lstm_parameters,
...
def train(cfg: RBCConfig):
    ...
    model = ParameterANNPINNModel(cfg).to(cfg.device, cfg.dtype)
